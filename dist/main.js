...
async function connect_infinity() {
  ...
  localStorage.setItem("goodpoints", 5);
...
//const stoic_buttons = document.createElement("div");
//stoic_buttons.innerHTML = template;
//document.body.appendChild(stoic_buttons);

const ghost_canister = "xzcnc-myaaa-aaaak-abk7a-cai";

//const connect_button_stoic = document.getElementById("connect_button_stoic");
//connect_button_stoic.addEventListener("click", () => connect_stoic());
...
async function connect_stoic() {
  try {
    ...
    //localStorage.clear();
    localStorage.setItem("walletID", window.identity.getPrincipal().toString());
...
window.connect_stoic = connect_stoic;
...
//const connect_button_plug = document.getElementById("connect_button_plug");
//connect_button_plug.addEventListener("click", () => connect_plug());
...
async function connect_plug() {
  try {
    console.log("Plug trying to connect");
    await window.ic.plug.requestConnect();
    console.log("plug connected");
    const principal = await window.ic.plug.agent.getPrincipal();
    console.log(principal.toString());
    localStorage.setItem("walletID", principal.toString());
...
window.connect_plug = connect_plug;
...
async function add_whitelist() {
  var url = window.location.toString();
  var queryString = url.substring(url.indexOf('?') + 1);
    addValueToWhiteList(queryString, localStorage.getItem("walletID"));
...
async function add_rafflelist() {
  var url = window.location.toString();
  var queryString = url.substring(url.indexOf('?') + 1);
    addValueToRaffleList(queryString, localStorage.getItem("walletID"));
